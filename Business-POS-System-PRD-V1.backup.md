...
8. **No full purchase-order (PO) module** in v1 — but the order-vs-received distinction is still modeled (§11).
9. **No formal AR/AP aging subledger** — only payables/receivables derivation (§15, §34).
...
## 6. Permission Model — single source of truth

### 6.1 Capability matrix (authoritative)
...
### 6.2 Design direction (RECOMMENDATION)
...
## 7. Editing Policy (transaction / key-record changes)

**AI ASSUMPTION — NEEDS CONFIRMATION.** Rules:

- **7.1 Draft transactions** — before posting. Freely editable; no stock/financial effect; no cost frozen.
- **7.2 Posted / completed transactions** — must **NOT** be edited in place for any field that affects inventory, revenue, COGS, payment, expenses, receivables, or payables. Corrections use the lifecycle: **Cancel/Void → reverse effects → create a corrected transaction**. Original + reversal remain visible in audit.
- **7.3 Non-financial fields** (e.g. a note) may be editable even after posting, and the edit is logged.
- **7.4 Why it preserves history:** prevents silent modification of immutable posted effects. Historical COGS and inventory are never recomputed from today's scores (§17).

**Status: AI ASSUMPTION — NEEDS CONFIRMATION** (owner decides: cancel/re-enter vs some in-place edits).

---

## 8. Transaction Lifecycle & Payment State (two separate axes)
...
| `Cancelled` / `Voided` | Invalidated; effects reversed exactly once (§13/§30). |
| `Returned` | Goods physically returned after a valid (not cancelled) transaction — separate from Cancellation. |

Per-document allowed sets are a **TBD** state matrix (§35).
...
### 8.3 The two axes do not conflict
...
## 9. Core Business Workflows (overview)

1. **Purchase flow** (§11)
2. **Sales / POS flow** (§12)
3. **Production** (§14)
4. **Stock adjustment** (§13)
5. **Cancellation / return** (§§11–13, §8)
6. **Manual income / expense** (§16)
7. **Permission delegation** (§6)
...
- B4. Purchasable/Sellable/Used-in-production flags are **RECOMMENDED** (§34). V1 default assumption: products are buyable and sellable and consumable unless a flag is added.
...
### Examples
Flour purchases Rp10k → 11k → 12k: card shows **Rp12,000**; a past sale at Rp10,000 stays Rp10,000 in history (§17).
...
- **P5.** **Stock increases only on goods receipt**, not merely when the purchase record is created. **AI ASSUMPTION — NEEDS CONFIRMATION** (aligns with the no-PO-module scope note in §3/§4; we still separate "ordered" from "received").
...
### Cancellation vs Return (§8 lifecycle)
...
**Refund/cash-return policy is NOT confirmed** — options (refund same method, issue credit/payable, offset) are open (§35).

### Inputs / Outputs / Validation
- Input: supplier (per §19 policy), line items (product, qty, price), totals, shipping, payments, status.
...
### Dependencies: §12 sales, §13 inventory, §15 payments, §19 supplier, §17 costing.
...
- Inventory check per negative-stock policy (§13).

### Success / failure
- Success: stock down, payment captured, optional customer receivable updated.
- Failure: insufficient stock → warning or block per §13.
...
### Inventory movement matrix — ONE authoritative effect per event (PART crucial)
| Event | Stock effect |
|---|---|
...
- Default block + warning. Owner-OPTIONAL enable negative (per-item/global). Negative rows clearly flagged and audited. COGS consequence: see §17.
...
## 14. Production / Manufacturing — V1 TEMPORARY model
...
### Cost-entry unit — TBD
Per production **run** (v1 default) vs per **period** (e.g. monthly). Flag in §35.
...
### Payment-state model
Payment axis (`Unpaid`/`Partial`/`Paid`) applies to sale/purchase (§8). Payments recorded as **allocations**; cash over-tender uses the tender/allocated/change triple (sales, §12).
...
### Refund behavior — **TBD**
Cancel/return involving prior payments: refund/credit handling is not defined/confirmed. Options open (same method refund, credit, offset). Nothing assumed as confirmed (§15, §35).
...
Support configurable methods: **FIFO**, **Average Cost**, **Last Purchase Price**, future. Recommended default = **Average Cost** (simple, no batch prerequisite). **NOT confirmed** — owner must select (§35).
...
### Note
Formal AR aging is future (§34 #9).
...
Not plain-CRUD tables. Every screen answers "what is the user trying to do?" Provide list/filter/search/sort/edit/detail/period-compare across modules. Editing the rules per §7/§8. Consistent UX. Status: confirmed philosophy.
...
### Editing summary
Draft freely; posted (stock/finance) not editable in place — cancel/re-enter (§7).
...
2. **Insufficient stock** (sale/purchase/production) — warning; hard-block per negative-stock config (§13).
...
| Editing posted sale | Cancel/re-enter per §7; historical cost stable. |
| Price-override | Permission-gated and logged. |
| Missing supplier/customer | Allowed per assumptions; report shows blank. |
| Void double-reversal | Prevented by inventory matrix (§13). |

---

## 29. Business Rules (Consolidated)
...
17. Gross = Reven − COGS / = Rev … and Net = GP − Expenses. **CONFIRMED**
...
21. Customer optional; supplier (m)confirm. **CONFIRMED(cust) / TBD(supplier)**
...
## 30. Status Matrix (Confirmed vs Assumption vs Rec vs TBD)
...
| Single-aware inventory correctness | AI ASSUMPTION |
| All KPIs single source | RECOMMENDED/CONFIRMED philosophy |

---

## 31. Open Questions / TBD (prioritized)
...
Out of V1: recipe/BOM, double-entry GL, payroll, multi-currency, tax engine, mobile/notif push, warehouses, uom, promo, batch/lot. Some in potential-future (§34).
...
## 34. Potential Future / Open Requirements (PART-12 added)
...
Also non-blocking: single-unit default/flag per product (sellable/purchasable flags) — see §10 B4.
...
12. Editing a posted sale in-place is rejected. (per §7 policy once confirmed).
...
## 36. Requirement Risk & Change Impact (DB/architecture)
...
| 6 | **Negative inventory** | MEDIUM | Optional. Related COGS rule is the open point (see §17); design config flag + guard; the schema is stable either way. |
...
## 36. Appendix — Conceptual Examples (all internally consistent)

| Topic | Numbers | Check |
|---|---|---|
| Purchases paid in full | 4M + 3M + 3M = 10M → Paid | ✔ |
| Sales over-tender | total 450k / tender 500k / alloc 450k / change 50k | ✔ |
| Average costing | avg ≈10,666.67; snapshot kept | §17 |
| P&L | 50M − 30M = GP 20M; − 8M = NP 12M (or valid) | ✔ |
| Production valuation (conceptual) | 135k / 10 = 13.5k (raw/base TBD) | §14 |
...
- Stale cross-references removed. ✔
