"""Production Inputs sub-resource API routes (M5 / Phase E.2).
...
from app.validation.production_schemas import ProductionInputRequest

logger = get_logger(__name__)

# Mounted under /production-runs so the full paths are
# /production-runs/{run_id}/inputs and /production-runs/{run_id}/inputs/{input_id}
router = APIRouter(prefix="/production-runs", tags=["production-inputs"])
...
@router.get(
    "/{run_id}/inputs",
    operation_id="listProductionInputs",
    summary="List production inputs for a run.",
    status_code=status.HTTP_200_OK,
)
async def list_production_inputs(
    request: Request,
    run_id: int,
    ...
    async with UnitOfWork() as uow:
        ...
        result = await svc.list_inputs(run_id)
        await uow.commit()
        pag = make_pagination(
...
@router.post(
    "/{run_id}/inputs",
    operation_id="addProductionInput",
    summary="Add a production input to a Draft run. Requires Idempotency-Key + If-Match.",
    status_code=status.HTTP_201_CREATED,
)
async def add_production_input(
    request: Request,
    run_id: int,
    ...
    if etag_parsed is None:
        raise MissingHeader(
            "If-Match header is required for POST /production-runs/{id}/inputs."
        )
    body_dict = await request.json()
    payload = ProductionInputRequest.model_validate(body_dict)
    async with UnitOfWork() as uow:
        svc = ProductionRunService(uow)
        try:
            result, is_replay = await svc.add_input(
                run_id=run_id,
...
@router.patch(
    "/{run_id}/inputs/{input_id}",
    operation_id="updateProductionInput",
    summary="Update a production input on a Draft run. Requires If-Match.",
    status_code=status.HTTP_200_OK,
)
async def update_production_input(
    request: Request,
    run_id: int,
    input_id: int,
    principal: Annotated[Principal, Depends(require_capability("production.edit_own_draft"))],
    ...
    if etag_parsed is None:
        raise MissingHeader(
            "If-Match header is required for PATCH /production-runs/{id}/inputs/{input_id}."
        )
    body_dict = await request.json()
    payload = ProductionInputRequest.model_validate(body_dict)
    async with UnitOfWork() as uow:
        svc = ProductionRunService(uow)
        result = await svc.update_input(
            run_id=run_id,
            input_id=input_id,
            principal_user_id=principal.user_id,
            input_data=payload.model_dump(mode="json"),
...
@router.delete(
    "/{run_id}/inputs/{input_id}",
    operation_id="deleteProductionInput",
    summary="Delete a production input from a Draft run. Requires Idempotency-Key.",
    status_code=status.HTTP_204_NO_CONTENT,
)
async def delete_production_input(
    request: Request,
    run_id: int,
    input_id: int,
    principal: Annotated[Principal, Depends(require_capability("production.edit_own_draft"))],
    ...
    async with UnitOfWork() as uow:
        svc = ProductionRunService(uow)
        _result, is_replay = await svc.delete_input(
            run_id=run_id,
            input_id=input_id,
            principal_user_id=principal.user_id,
            idempotency_key=str(parsed_key.value),
