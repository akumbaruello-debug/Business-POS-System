...
from typing import Any  # noqa: F401  (used by F.1 helpers below)
...
# ============================================================
# F.1 — module-level helpers used by the report-endpoint builder
# below. Defined here so they can be referenced by both the F.1
# `report_endpoints` builder and any future F.* report entry that
# wants the same level of parameter-set control.
# ============================================================
from collections import namedtuple  # noqa: E402

_ReportEndpoint = namedtuple(
    "_ReportEndpoint",
    ["slug", "op_id", "desc", "schema", "capability", "params"],
)

_PERIOD_PARAM = {"$ref": "#/components/parameters/Period"}
_FROM_PARAM = {"$ref": "#/components/parameters/From"}
_TO_PARAM = {"$ref": "#/components/parameters/To"}
_COMPARE_TO_PARAM = {"$ref": "#/components/parameters/CompareTo"}


def _FILTER_PARAM(name: str, alias: str) -> dict[str, Any]:
    """Build a ``filter[name]`` query-parameter block for an endpoint.

    The schema is intentionally loose (string OR integer) so the
    same shape works for integer-valued filters (supplier_id),
    enum-valued filters (lifecycle_status), and the legacy
    string-valued ``filter[product_id]`` (the E.7 convention uses
    integer via Query). OpenAPI consumers treat ``filter[X]`` as an
    opaque server-side filter key.
    """
    return {
        "name": name,
        "in": "query",
        ...
        "description": f"Filter parameter (alias: `{alias}`).",
    }
...
def post_op(
    ...
    if responses_200:
        responses["200"] = {
            ...
            "content": {
                "application/json": {
                    "schema": {"$ref": f"#/components/schemas/{responses_200}"}
                }
            },
        }
    if responses_201:
        responses["201"] = {
            ...
            "content": {
                "application/json": {
                    "schema": {"$ref": f"#/components/schemas/{responses_201}"}
                }
            },
        }
    if responses_204:
...
def collection_get_op(
    op_id,
    summary,
    item_schema,
    tags,
    capability,
    filterable_extra=None,
...
def build_schemas():
        ...
        "properties": {
            "error": {
                ...
                "properties": {
                    ...
                    "details": {
                        "description": "Optional, code-specific structure.",
                        "oneOf": [
                    ...
                    "request_id": {
                        "type": "string",
                        "format": "uuid",
                        "description": "Echoes `X-Request-ID` header.",
                    },
                },
            },
        },
                        ...
                        "properties": {
                            "code": {
                                ...
                                "enum": [
                                    ...
                                    "allocation_exceeds_payable",
                                    "refund_exceeds_crl",
                                    "repayment_exceeds_srec",
                                    "cash_insufficient",
                                    "return_quantity_exceeds_original",
                                    "purchase_return_quantity_exceeds_original",
                                    "lifecycle_state_invalid",
                                    "already_cancelled",
                                    "cannot_cancel_after_return",
                                    "production_inputs_exceed_stock",
                                    "production_finished_cost_invalid",
                                    "cost_type_deactivated",
                                    "product_deactivated",
    ...
    s["RoleCapabilitiesRequest"] = {
        "type": "object",
        "required": ["capability_codes"],
        "properties": {
            "capability_codes": {"type": "array", "items": {"type": "string"}},
        },
    }
    s["Capability"] = {
        "type": "object",
        "required": ["id", "code", "domain", "default_owner", "default_staff"],
        "properties": {
            "id": {"type": "integer", "minimum": 1},
            "code": {"type": "string"},
            "domain": {"type": "string"},
    ...
    s["UserCreateRequest"] = {
        "type": "object",
        "required": ["username", "full_name", "password", "role_id"],
        "properties": {
            "username": {
                "type": "string",
                "minLength": 1,
    ...
    s["UserCapabilityOverrideRequest"] = {
        "type": "object",
        "required": ["capability_code", "is_granted"],
        "properties": {
            "capability_code": {"type": "string"},
            "is_granted": {"type": "boolean"},
        },
    }
    s["UserEffectiveCapabilities"] = {
        "type": "object",
        "required": ["role", "role_capabilities", "overrides", "effective"],
        "properties": {
            "role": {"type": "string"},
    ...
    s["AuditEntry"] = {
        "type": "object",
        "required": ["id", "event_time", "action", "entity_type"],
        "properties": {
    ...
    for (
        ...
        s[schema_name] = {
            "type": "object",
            "required": ["id", "name", "is_active", "version"],
            "properties": {
                "id": {"type": "integer", "minimum": 1},
                "name": {
                    "type": "string",
        ...
        if prefix in ("Category",):
            s[schema_name]["properties"].update(
                {"parent_id": {"type": ["integer", "null"], "minimum": 1}}
            )
            s[schema_name]["required"] = [
                "id",
                "name",
                "is_active",
                "created_at",
        ...
        if prefix in ("Unit", "CostType", "FinancialCategory"):
            s[schema_name]["required"] = (
                ["id", "code", "name", "is_active", "version"]
                if prefix != "FinancialCategory"
                else ["id", "code", "name", "entry_type", "is_active", "version"]
            )
            s[schema_name]["properties"].update(
                {
                    "code": {
                        "type": "string",
                        "minLength": 1,
                        "maxLength": 50 if prefix != "Unit" else 20,
                        "pattern": pat,
                    }
                }
            )
        ...
        if prefix == "Category":
            s[req_name]["properties"].update(
                {"parent_id": {"type": ["integer", "null"], "minimum": 1}}
            )
        if prefix in ("Unit", "CostType", "FinancialCategory"):
            s[req_name]["required"] = ["code", "name"]
            s[req_name]["properties"]["code"] = {
                "type": "string",
                "minLength": 1,
                "maxLength": 50 if prefix != "Unit" else 20,
                "pattern": pat,
        ...
        if prefix == "FinancialCategory":
            ...
            s[req_name]["properties"]["entry_type"] = {
                "type": "string",
                "enum": ["income", "expense"],
            }
        ...
        if prefix == "Category":
            s[patch_name]["properties"].update(
                {"parent_id": {"type": ["integer", "null"], "minimum": 1}}
            )
    ...
    s["Product"] = {
        ...
        "properties": {
            ...
            "on_hand_quantity": {
                "type": "number",
                "description": "Derived: SUM(stock_movements.quantity).",
            },
            "moving_average_unit_cost": {
                "type": "number",
                "description": "Derived: SUM(positive total_cost) / on_hand_qty.",
            },
            "inventory_value": {
                "type": "number",
                "description": "Derived: on_hand × moving_avg.",
            },
    ...
    s["ProductPatch"] = {
        "type": "object",
        "description": "PATCH on a product master. `code` is immutable. Price changes insert a price_history row; historical transactions are unchanged.",
        "properties": {
            "name": {"type": "string", "minLength": 1, "maxLength": 200},
            "category_id": {"type": ["integer", "null"], "minimum": 1},
            "unit_id": {"type": ["integer", "null"], "minimum": 1},
    ...
    s["ProductValuation"] = {
        "type": "object",
        "required": ["product_id", "on_hand_quantity", "inventory_value", "as_of"],
        "properties": {
            "product_id": {"type": "integer"},
    ...
    s["Sale"] = {
        ...
        "properties": {
            ...
            "total_amount": {
                "type": "number",
                "minimum": 0,
                "description": "Σ line_total − header discount.",
            },
    ...
    s["SaleLineInput"] = {
        ...
        "properties": {
            ...
            "unit_price": {
                "type": "number",
                "minimum": 0,
                "description": "Defaults to product.selling_price. Override requires `sale.price_override`; logged in audit.",
            },
            "discount_amount": {
                "type": "number",
                "minimum": 0,
                "description": "Non-zero requires `sale.discount`.",
            },
        },
    }
    s["SaleLine"] = {
        ...
        "properties": {
            ...
            "line_total": {
                "type": "number",
                "minimum": 0,
                "description": "Server-computed: quantity × unit_price − discount_amount.",
            },
            "unit_cost_snapshot": {
                "type": ["number", "null"],
                "description": "Server-computed at post; immutable once posted.",
            },
    ...
    s["SaleCreateRequest"] = {
        ...
        "properties": {
            "customer_id": {
                "type": ["integer", "null"],
                "minimum": 1,
                "description": "Optional. Customer is never required.",
            },
            "sale_date": {"type": "string", "format": "date-time"},
    ...
    s["SalePaymentInput"] = {
        ...
        "properties": {
            ...
            "tendered_amount": {
                "type": ["number", "null"],
                "minimum": 0,
                "description": "Required only for over-tender cash; allowed only when `payment_method.is_cash` is true.",
            },
            "change_amount": {
                "type": ["number", "null"],
                "minimum": 0,
                "description": "Server-computed.",
            },
    ...
    s["SaleReturnRequest"] = {
        ...
        "properties": {
            ...
            "lines": {
                "type": "array",
                "minItems": 1,
                "items": {"$ref": "#/components/schemas/SaleReturnLineRequest"},
            },
        },
    }
    s["SalesReturn"] = {
        ...
        "properties": {
            ...
            "lines": {
                "type": "array",
                "items": {"$ref": "#/components/schemas/SalesReturnLine"},
            },
            "created_at": {"type": "string", "format": "date-time"},
            "created_by": {"type": "integer"},
        },
    ...
    s["SalesReturnLine"] = {
        ...
        "properties": {
            ...
            "returned_selling_price": {
                "type": "number",
                "description": "Server-computed: sale_line.unit_price × quantity.",
            },
            "returned_unit_cost": {
                "type": "number",
                "description": "Server-computed from sale_line.unit_cost_snapshot.",
            },
            "line_value": {
                "type": "number",
                "description": "= returned_selling_price.",
            },
            "line_number": {"type": "integer", "minimum": 1},
        },
    }
    s["SalesReturnCancelRequest"] = {
        "type": "object",
        "required": ["reason"],
        "properties": {"reason": {"type": "string", "minLength": 1, "maxLength": 1000}},
    }
    ...
    s["RefundRequest"] = {
        "type": "object",
        "required": ["sale_id", "amount", "payment_method_id"],
        "properties": {
            "sale_id": {"type": "integer", "format": "int64"},
            "amount": {
                "type": "number",
                "exclusiveMinimum": 0,
                "description": "Must be ≤ current CRL (Σ payments − Σ refunds) and ≤ current cash balance.",
            },
            "payment_method_id": {"type": "integer", "minimum": 1},
    ...
    s["Purchase"] = {
        ...
        "properties": {
            ...
            "supplier_id": {
                "type": ["integer", "null"],
                "minimum": 1,
                "description": "Optional. Counter/cash buys allowed.",
            },
            ...
            "total_amount": {
                "type": "number",
                "minimum": 0,
                "description": "Σ line_total + shipping.amount.",
            },
            "lifecycle_status": {
                "type": "string",
                "enum": [
            ...
            "lines": {
                "type": "array",
                "items": {"$ref": "#/components/schemas/PurchaseLine"},
            },
            "shipping": {"$ref": "#/components/schemas/PurchaseShipping"},
            "payments": {
                "type": "array",
                "items": {"$ref": "#/components/schemas/PurchasePayment"},
            },
            "returns": {
                "type": "array",
    ...
    s["PurchaseShipping"] = {
        "type": "object",
        "required": ["id", "purchase_id", "amount", "paid_in_cash"],
        "properties": {
            "id": {"type": "integer", "format": "int64"},
            "purchase_id": {"type": "integer", "format": "int64"},
            "amount": {
                "type": "number",
                "minimum": 0,
                "description": "Always capitalized into landed cost.",
            },
            "paid_in_cash": {
                "type": "boolean",
                "description": "TRUE → cash_movement at post (Dr Inventory / Cr Cash). FALSE → AP.",
            },
            "supplier_id": {"type": ["integer", "null"]},
            "description": {"type": ["string", "null"]},
        },
    }
    s["PurchaseShippingRequest"] = {
        "type": "object",
        "required": ["amount", "paid_in_cash"],
        "properties": {
    ...
    s["PurchaseCreateRequest"] = {
        ...
        "properties": {
            ...
            "lines": {
                "type": "array",
                "minItems": 1,
                "items": {"$ref": "#/components/schemas/PurchaseLineInput"},
            },
    ...
    s["PurchasePaymentInput"] = {
        "allOf": [
            {"$ref": "#/components/schemas/SalePaymentInput"},
            {
                "type": "object",
                "description": "Same shape as sale payment. Over-tender only for cash methods.",
            },
        ]
    }
    s["PurchasePayment"] = {
    ...
    s["PurchaseCancelRequest"] = {
        "type": "object",
        "required": ["reason"],
        "properties": {"reason": {"type": "string", "minLength": 1, "maxLength": 1000}},
    }
    s["PurchaseReturnLineRequest"] = {
        "type": "object",
        "required": ["purchase_line_id", "quantity"],
        "properties": {
            "purchase_line_id": {"type": "integer", "format": "int64"},
            "quantity": {"type": "number", "exclusiveMinimum": 0, "maximum": 1e12},
        },
    }
    s["PurchaseReturnRequest"] = {
        "type": "object",
        "required": ["reason", "lines"],
        "properties": {
            "reason": {"type": "string", "minLength": 1, "maxLength": 1000},
            "lines": {
                "type": "array",
                "minItems": 1,
                "items": {"$ref": "#/components/schemas/PurchaseReturnLineRequest"},
            },
        },
    }
    s["PurchaseReturn"] = {
    ...
    s["SupplierRepaymentRequest"] = {
        ...
        "properties": {
            ...
            "amount": {
                "type": "number",
                "exclusiveMinimum": 0,
                "description": "≤ outstanding and ≤ current cash balance.",
            },
            "payment_method_id": {"type": "integer", "minimum": 1},
    ...
    s["StockMovement"] = {
        ...
        "properties": {
            ...
            "quantity": {
                "type": "number",
                "description": "0 only for `value_adjustment`.",
            },
    ...
    s["StockAdjustmentRequest"] = {
        ...
        "properties": {
            ...
            "quantity": {
                "type": "number",
                "exclusiveMinimum": -1e12,
                "maximum": 1e12,
                "description": "Signed delta; ≠ 0.",
            },
            "reason": {"type": "string", "minLength": 1, "maxLength": 1000},
        },
    }
    ...
    s["ProductionRun"] = {
        ...
        "properties": {
            ...
            "inputs": {
                "type": "array",
                "items": {"$ref": "#/components/schemas/ProductionInput"},
            },
            "output": {"$ref": "#/components/schemas/ProductionOutput"},
            "cost_lines": {
                "type": "array",
                "items": {"$ref": "#/components/schemas/ProductionCostLine"},
            },
            "created_at": {"type": "string", "format": "date-time"},
            "updated_at": {"type": "string", "format": "date-time"},
            "created_by": {"type": "integer"},
    ...
    s["ProductionOutput"] = {
        ...
        "properties": {
            ...
            "unit_cost_snapshot": {
                "type": "number",
                "minimum": 0,
                "description": "= parent.finished_unit_cost.",
            },
            "total_cost": {"type": "number", "minimum": 0},
        },
    }
    s["ProductionCostLine"] = {
        "type": "object",
        "required": [
            "id",
            "production_run_id",
    ...
    s["ProductionCostLineRequest"] = {
        "type": "object",
        "required": ["cost_type_id", "amount"],
        "properties": {
    ...
    s["ProductionRunCreateRequest"] = {
        "type": "object",
        "required": ["output_product_id", "output_quantity", "inputs"],
        "properties": {
            "run_date": {"type": "string", "format": "date-time"},
            ...
            "inputs": {
                "type": "array",
                "minItems": 1,
                "items": {"$ref": "#/components/schemas/ProductionInputRequest"},
            },
            "cost_lines": {
                "type": "array",
                "items": {"$ref": "#/components/schemas/ProductionCostLineRequest"},
            },
        },
    }
    s["ProductionRunPatch"] = {
        "type": "object",
        "properties": {
            "run_date": {"type": "string", "format": "date-time"},
            "output_product_id": {"type": "integer", "minimum": 1},
    ...
    # -------- Manual entries --------
    s["ManualEntryRequest"] = {
        "type": "object",
        "required": ["category_id", "entry_type", "amount", "payment_method_id"],
        "properties": {
            "category_id": {
                "type": "integer",
                "minimum": 1,
                "description": "Must match `entry_type`. Category name may not duplicate derived categories.",
            },
            "entry_type": {"type": "string", "enum": ["income", "expense"]},
            "amount": {"type": "number", "exclusiveMinimum": 0, "maximum": 1e13},
            "payment_method_id": {"type": "integer", "minimum": 1},
    ...
    s["CashMovement"] = {
        ...
        "properties": {
            ...
            "amount": {
                "type": "number",
                "description": "Signed: positive for inflows, negative for outflows.",
            },
    ...
    s["CashBalance"] = {
        "type": "object",
        "required": ["balance", "as_of"],
        "properties": {
            "balance": {
                "type": "number",
                "description": "Derived: SUM(cash_movements.amount).",
            },
            "as_of": {"type": "string", "format": "date-time"},
        },
    }
    ...
    s["SettingsMap"] = {
        "type": "object",
        "description": "Map of system setting key → value. `costing_method` is read-only in V1 (returns 400 on PATCH).",
        ...
        "example": {
            ...
            "default_negative_stock_allowed": False,
        },
    }
    s["SettingsPatch"] = {
        "type": "object",
        "description": "Partial update. Each key is validated against the known set.",
        "additionalProperties": {
            "oneOf": [
                {"type": "string"},
                {"type": "number"},
    ...
    s["SystemVersionResponse"] = {
        "type": "object",
        "required": ["version", "api_version", "server_time"],
        "properties": {
            "version": {"type": "string"},
    ...
    # F.1 — purchasesReport response: paged list + optional comparison
    # (D-7 — architecture says purchases supports comparison).
    s["PurchasesReportResponse"] = {
        "type": "object",
        "required": ["data", "pagination"],
        "description": (
            "F.1 purchases report response. Extends the standard "
            "PagedResponse with an optional ``comparison`` block per "
            "D-7 (architecture §12.1 line 1092 lists purchases among "
            "the reports that support period comparison)."
        ),
        "properties": {
            "data": {
                "type": "array",
            ...
            "pagination": {"$ref": "#/components/schemas/Pagination"},
            "comparison": {
                "allOf": [{"$ref": "#/components/schemas/PeriodComparison"}],
                "nullable": True,
            },
        },
    }
...
def build_components():
    ...
    c["parameters"] = {
        ...
        "PerPage": {
            ...
            "description": "Page size. Hard cap 500.",
        },
        "Sort": {
            "name": "sort",
            "in": "query",
            "required": False,
            "schema": {"type": "string"},
            "description": "Comma-separated list. Prefix `-` for descending.",
        },
        "Q": {
            "name": "q",
            "in": "query",
        ...
        "Period": {
            ...
            "description": "Convenience period alias.",
        },
        "CompareTo": {
            "name": "compare_to",
            "in": "query",
            "required": False,
            "schema": {"type": "string"},
            "description": "Period to compare against: `period` alias or `from,to`.",
        },
        "Include": {
            "name": "include",
            "in": "query",
            "required": False,
            "schema": {"type": "string"},
            "description": "Comma-separated sub-resources to embed (e.g., `lines,payments,returns`).",
        },
    }
    c["responses"] = {
        "BadRequest": {
        ...
        "Unauthorized": {
            "description": "Missing, invalid, or expired credentials (401).",
            "content": {
                "application/json": {
        ...
        "Forbidden": {
            "description": "Authenticated but missing required capability (403).",
            "content": {
                "application/json": {
                    "schema": {
                        "$ref": "#/components/schemas/AuthorizationErrorEnvelope"
                    }
                }
            },
        },
        ...
        "Conflict": {
            ...
            "content": {
                "application/json": {
                    "schema": {"$ref": "#/components/schemas/ConflictErrorEnvelope"}
                }
            },
        },
        "PreconditionFailed": {
            "description": "If-Match ETag mismatch (412).",
            "content": {
                "application/json": {
                    "schema": {"$ref": "#/components/schemas/ErrorEnvelope"}
                }
        ...
        "UnprocessableEntity": {
            "description": "Semantic conflict (422).",
            "content": {
                "application/json": {
                    "schema": {"$ref": "#/components/schemas/ErrorEnvelope"}
                }
        ...
        "InternalServerError": {
            "description": "Server-side error (500).",
            "content": {
                "application/json": {
                    "schema": {"$ref": "#/components/schemas/ErrorEnvelope"}
                }
        ...
        "ServiceUnavailable": {
            "description": "Maintenance or transient (503).",
            "content": {
                "application/json": {
                    "schema": {"$ref": "#/components/schemas/ErrorEnvelope"}
                }
...
def build_paths():
    ...
    p["/auth/login"] = {
        "post": post_op(
            "loginUser",
            "Authenticate user, issue access + refresh tokens. Required `Idempotency-Key` (30s TTL) prevents replay.",
            tags=["Auth"],
            ...
            response_codes={
                ...
                423: (
                    "Locked",
                    "ErrorEnvelope",
                    "account_locked (5th attempt within 15 min)",
                ),
    ...
    p["/auth/refresh"] = {
        "post": post_op(
            "refreshToken",
            "Rotate access and refresh tokens. Old refresh is invalidated.",
            tags=["Auth"],
    ...
    p["/auth/me"] = {
        "get": get_op(
            "getCurrentUser",
            "Return current user with effective capabilities.",
            "SessionUser",
            tags=["Auth"],
        )
    }
    ...
    p["/users"] = {
        ...
        "post": post_op(
            ...
            idempotency_required=True,
            response_codes={
                409: ("Conflict", "ErrorEnvelope", "username_exists / email_exists")
            },
        ),
    }
    p["/users/{id}"] = {
        ...
        "get": get_op(
            "getUser", "Get user.", "User", tags=["Users"], capability="user.view"
        ),
        "patch": patch_op(
            "updateUser",
            "Update user (full_name, email, is_active, role_id).",
            tags=["Users"],
            capability="user.manage",
            request_schema="UserUpdateRequest",
            if_match_required=True,
            response_codes={
                412: ("Precondition Failed", "ErrorEnvelope", "version_mismatch")
            },
        ),
    }
    p["/users/{id}/deactivate"] = {
        "post": post_op(
            "deactivateUser",
            "Deactivate user. Revokes all sessions.",
            tags=["Users"],
            capability="user.manage",
            has_path_id=True,
            responses_204=None,
            idempotency_required=True,
            response_codes={
                403: ("Forbidden", "ErrorEnvelope", "user is the only Owner")
            },
        )
    }
    p["/users/{id}/reset-password"] = {
        "post": post_op(
            "resetUserPassword",
    ...
    p["/users/{id}/capabilities"] = {
        "parameters": [path_id_("id", "User id")],
        "get": get_op(
            "listUserEffectiveCapabilities",
            "List role + overrides + effective.",
        ...
        "post": post_op(
            "grantUserCapability",
            "Grant or deny a capability for a user.",
            tags=["Users"],
            capability="user.grant_capability",
            ...
            response_codes={
                400: (
                    "Bad Request",
                    "ErrorEnvelope",
                    "enum_value_invalid (unknown capability)",
                )
            },
        ),
    }
    p["/users/{id}/capabilities/{capability_code}"] = {
        ...
        "delete": delete_op(
            ...
            "Revoke capability override for a user. Effect is immediate.",
            tags=["Users"],
            capability="user.revoke_capability",
            idempotency_required=True,
        ),
    }
    ...
    p["/roles"] = {
        ...
        "post": post_op(
            "createRole",
            "Create role. Starts with zero capabilities.",
            tags=["Roles"],
            capability="role.manage",
            request_schema="RoleCreateRequest",
            responses_201="Role",
            idempotency_required=True,
        ),
    }
    p["/roles/{id}"] = {
        ...
        "get": get_op(
            "getRole", "Get role.", "Role", tags=["Roles"], capability="role.view"
        ),
        "patch": patch_op(
            "updateRole",
            ...
            tags=["Roles"],
            capability="role.manage",
            request_schema="RoleUpdateRequest",
            if_match_required=True,
            response_codes={
        ...
        "delete": delete_op(
            ...
            response_codes={
                ...
                409: (
                    "Conflict",
                    "ErrorEnvelope",
                    "referenced_by_history (users assigned)",
                ),
            },
        ),
    }
    p["/roles/{id}/capabilities"] = {
        ...
        "put": put_op(
            "setRoleCapabilities",
            "Replace the role's full capability set.",
            tags=["Roles"],
            capability="role.manage",
            request_schema="RoleCapabilitiesRequest",
            responses_200="Role",
            idempotency_required=True,
            response_codes={
                403: ("Forbidden", "ErrorEnvelope", "system_role_immutable")
    ...
    p["/capabilities"] = {
        "get": collection_get_op(
            "listCapabilities",
            "List canonical capability codes.",
            "Capability",
            tags=["Capabilities"],
            capability="user.view",
    ...
    for resource, tag, schema, req_schema, patch_schema, cap in [
        ...
        p[f"/{resource}"] = {
            ...
            "post": post_op(
                f"create{tag.replace(' ', '')}",
                f"Create {singular.lower()}.",
                tags=[tag],
                capability=cap_manage,
                request_schema=req_schema,
                responses_201=schema,
                idempotency_required=True,
            ),
        }
        # single
        p[f"/{resource}/{{id}}"] = {
            ...
            "get": get_op(
                f"get{tag.replace(' ', '')}",
                f"Get {singular.lower()}.",
                schema,
                tags=[tag],
                capability=cap_view,
            ),
            "patch": patch_op(
                f"update{tag.replace(' ', '')}",
                f"Update {singular.lower()}.",
                tags=[tag],
                capability=cap_manage,
            ...
            "delete": delete_op(
                f"delete{tag.replace(' ', '')}",
                f"Hard delete {singular.lower()} (only if unreferenced).",
                tags=[tag],
                capability=cap_manage,
                idempotency_required=True,
                response_codes={
                    409: ("Conflict", "ErrorEnvelope", "referenced_by_history")
                },
            ),
        }
        p[f"/{resource}/{{id}}/deactivate"] = {
            "post": post_op(
                f"deactivate{tag.replace(' ', '')}",
                f"Deactivate {singular.lower()}. Preserves history.",
                tags=[tag],
                capability=cap_manage,
                has_path_id=True,
                request_schema="DeactivateRequest",
                responses_204=None,
                idempotency_required=True,
            ),
        }
    ...
    p["/products"] = {
        "get": collection_get_op(
            "listProducts",
            "List products (with derived stock fields).",
            "Product",
            tags=["Products"],
    ...
    p["/products/{id}"] = {
        ...
        "get": get_op(
            "getProduct",
            "Get product with last 10 price-history rows.",
            "Product",
            tags=["Products"],
            capability="product.view",
        ),
        ...
        "delete": delete_op(
            "deleteProduct",
            "Hard delete (only if unreferenced).",
            tags=["Products"],
    ...
    p["/contacts/{id}"] = {
        ...
        "patch": patch_op(
            ...
            "Update contact.",
            tags=["Contacts"],
            capability="contact.edit",
            request_schema="ContactPatch",
            if_match_required=True,
        ),
        "delete": delete_op(
            "deleteContact",
            "Hard delete contact (only if unreferenced).",
            tags=["Contacts"],
    ...
    p["/sales"] = {
        ...
        "post": post_op(
            "createSale",
            "Create sale in Draft. Server computes line_total and total_amount.",
            tags=["Sales"],
            capability="sale.create",
            request_schema="SaleCreateRequest",
            responses_201="Sale",
            idempotency_required=True,
        ),
    }
    p["/sales/{id}"] = {
        ...
        "patch": patch_op(
            ...
            tags=["Sales"],
            capability="sale.edit_own_draft",
            request_schema="SalePatch",
            if_match_required=True,
            response_codes={
                403: (
                    "Forbidden",
                    "ErrorEnvelope",
                    "lifecycle_state_invalid (not draft)",
                )
            },
        ),
        "delete": delete_op(
            "deleteSaleDraft",
            "Hard delete a Draft sale (only if no effects).",
            tags=["Sales"],
            capability="sale.create",
            idempotency_required=True,
            response_codes={
    ...
    p["/sales/{id}/lines"] = {
        ...
        "get": collection_get_op(
            "listSaleLines",
            "List lines for a sale.",
            "SaleLine",
            tags=["Sale Lines"],
            capability="sale.view",
            sortable=["line_number"],
        ),
        "post": post_op(
            "addSaleLine",
            "Add a line to a Draft sale. Mutates parent draft totals; requires If-Match.",
            tags=["Sale Lines"],
            capability="sale.edit_own_draft",
            request_schema="SaleLineInput",
            responses_201="SaleLine",
            idempotency_required=True,
            if_match_required=True,
        ),
    }
    p["/sales/{id}/lines/{line_id}"] = {
        "parameters": [
            path_id_("id", "Sale id", "int64"),
            path_id_("line_id", "Line id", "int64"),
        ],
        "patch": patch_op(
            "updateSaleLine",
            "Update a Draft sale line.",
            tags=["Sale Lines"],
            capability="sale.edit_own_draft",
            request_schema="SaleLinePatch",
            if_match_required=True,
        ),
        "delete": delete_op(
            "deleteSaleLine",
            "Delete a Draft sale line.",
            tags=["Sale Lines"],
            capability="sale.edit_own_draft",
            idempotency_required=True,
        ),
    ...
    p["/sales/{id}/payments"] = {
        ...
        "get": collection_get_op(
            "listSalePayments",
            "List payments on a sale.",
            "SalePayment",
            tags=["Sale Payments"],
            capability="sale.view",
            sortable=["payment_date", "id"],
        ),
        "post": post_op(
            "addSalePayment",
            ...
            capability="sale.create",
            has_path_id=True,
            request_schema="SalePaymentInput",
            responses_201="SalePayment",
            idempotency_required=True,
            if_match_required=True,
            response_codes={
                400: (
                    "Bad Request",
                    "ErrorEnvelope",
                    "tendered_not_allowed_for_non_cash / change_amount_mismatch",
                ),
                409: (
                    "Conflict",
                    "ConflictErrorEnvelope",
                    "payment_exceeds_total / cash_insufficient / version_mismatch",
                ),
            },
        ),
    }
    p["/sales/{id}/cancel"] = {
        "post": post_op(
            "cancelSale",
            "Cancel a posted/completed/returned sale. Reverses stock + revenue; no cash movement (CRL preserved).",
            tags=["Sales"],
            capability="sale.cancel",
            has_path_id=True,
            request_schema="SaleCancelRequest",
            responses_200="Sale",
            idempotency_required=True,
            if_match_required=True,
            response_codes={
                409: (
                    "Conflict",
                    "ConflictErrorEnvelope",
                    "already_cancelled / lifecycle_state_invalid / version_mismatch",
                )
            },
        )
    }
    p["/sales/{id}/returns"] = {
        ...
        "post": post_op(
            "createSaleReturn",
            "Process a return: increase stock at original cost; update lifecycle.",
            tags=["Sales Returns"],
            capability="sale.return",
            has_path_id=True,
            request_schema="SaleReturnRequest",
            responses_201="SalesReturn",
            idempotency_required=True,
            if_match_required=True,
            response_codes={
                409: (
    ...
    p["/sales-returns"] = {
        "get": collection_get_op(
            ...
            filterable_extra={
                ...
                "filter[customer_id]": {"schema": {"type": "integer"}},
                "filter[lifecycle_status]": {
                    "schema": {"type": "string", "enum": ["posted", "cancelled"]}
                },
            },
            sortable=["return_date", "id"],
        )
    ...
    p["/sales-returns/{id}/cancel"] = {
        "post": post_op(
            "cancelSalesReturn",
            "Cancel a sales return: insert reversal movements, update lifecycle.",
            tags=["Sales Returns"],
            capability="sale.return",
            has_path_id=True,
            request_schema="SalesReturnCancelRequest",
            responses_200="SalesReturn",
            idempotency_required=True,
            if_match_required=True,
        )
    }
    ...
    p["/refunds"] = {
        "get": collection_get_op(
            "listRefunds",
            "List refunds (customer disbursements). Refunds are cash-out, never income.",
            "Refund",
            tags=["Refunds"],
            capability="sale.view",
            filterable_extra={
    ...
    p["/purchases"] = {
        ...
        "post": post_op(
            "createPurchase",
            "Create purchase in Draft. Supplier optional (counter/cash buys allowed).",
            tags=["Purchases"],
            capability="purchase.create",
            request_schema="PurchaseCreateRequest",
            responses_201="Purchase",
            idempotency_required=True,
        ),
    }
    p["/purchases/{id}"] = {
        ...
        "get": get_op(
            "getPurchase",
            "Get purchase. Use `?include=lines,shipping,payments,returns`.",
            "Purchase",
            tags=["Purchases"],
            capability="purchase.view",
        ),
        "patch": patch_op(
            ...
            tags=["Purchases"],
            capability="purchase.edit_own_draft",
            request_schema="PurchasePatch",
            if_match_required=True,
            response_codes={
                403: (
                    "Forbidden",
    ...
    p["/purchases/{id}/lines"] = {
        ...
        "post": post_op(
            "addPurchaseLine",
            "Add a Draft purchase line. Mutates draft totals; requires If-Match.",
            tags=["Purchase Lines"],
            capability="purchase.edit_own_draft",
            request_schema="PurchaseLineInput",
            responses_201="PurchaseLine",
            idempotency_required=True,
            if_match_required=True,
        ),
    }
    p["/purchases/{id}/lines/{line_id}"] = {
        "parameters": [
            path_id_("id", "Purchase id", "int64"),
            path_id_("line_id", "Purchase line id", "int64"),
        ],
        "patch": patch_op(
            "updatePurchaseLine",
            "Update a Draft purchase line.",
            tags=["Purchase Lines"],
            capability="purchase.edit_own_draft",
            request_schema="PurchaseLinePatch",
            if_match_required=True,
        ),
        "delete": delete_op(
            "deletePurchaseLine",
            "Delete a Draft purchase line.",
            tags=["Purchase Lines"],
            capability="purchase.edit_own_draft",
            idempotency_required=True,
        ),
    ...
    p["/purchases/{id}/shipping"] = {
        ...
        "post": post_op(
            ...
            tags=["Purchase Shipping"],
            capability="purchase.edit_own_draft",
            has_path_id=True,
            request_schema="PurchaseShippingRequest",
            responses_200="PurchaseShipping",
            idempotency_required=True,
            if_match_required=True,
        ),
        "patch": patch_op(
            "updatePurchaseShipping",
            "Update shipping on a Draft purchase.",
            tags=["Purchase Shipping"],
            capability="purchase.edit_own_draft",
            request_schema="PurchaseShippingRequest",
            if_match_required=True,
    ...
    p["/purchases/{id}/post"] = {
        "post": post_op(
            ...
            tags=["Purchases"],
            capability="purchase.post",
            has_path_id=True,
            responses_200="Purchase",
            idempotency_required=True,
            if_match_required=True,
            response_codes={
                409: (
                    "Conflict",
                    "ConflictErrorEnvelope",
                    "lifecycle_state_invalid / version_mismatch",
                )
            },
        )
    }
    p["/purchases/{id}/payments"] = {
        ...
        "post": post_op(
            ...
            capability="purchase.create",
            has_path_id=True,
            request_schema="PurchasePaymentInput",
            responses_201="PurchasePayment",
            idempotency_required=True,
            if_match_required=True,
            response_codes={
                400: (
                ...
                409: (
                    "Conflict",
                    "ConflictErrorEnvelope",
                    "allocation_exceeds_payable / cash_insufficient / version_mismatch",
                ),
            },
        ),
    }
    p["/purchases/{id}/cancel"] = {
        "post": post_op(
            "cancelPurchase",
            ...
            has_path_id=True,
            request_schema="PurchaseCancelRequest",
            responses_200="Purchase",
            idempotency_required=True,
            if_match_required=True,
            response_codes={
    ...
    p["/purchases/{id}/returns"] = {
        ...
        "post": post_op(
            ...
            "Return goods to a supplier. Stock decreases at original landed cost.",
            tags=["Purchase Returns"],
            capability="purchase.return",
            has_path_id=True,
            request_schema="PurchaseReturnRequest",
            responses_201="PurchaseReturn",
            idempotency_required=True,
            if_match_required=True,
            response_codes={
                409: (
    ...
    p["/purchase-returns/{id}/cancel"] = {
        "post": post_op(
            "cancelPurchaseReturn",
            "Cancel a purchase return: insert reversal movements, update lifecycle.",
            tags=["Purchase Returns"],
            capability="purchase.return",
            has_path_id=True,
            request_schema="PurchaseReturnCancelRequest",
    ...
    p["/supplier-repayments"] = {
        ...
        "post": post_op(
            "createSupplierRepayment",
            "Record cash received from a supplier. Mutates parent purchase (SRec) and cash ledger; requires If-Match.",
            tags=["Supplier Repayments"],
            capability="purchase.refund",
            request_schema="SupplierRepaymentRequest",
            responses_201="SupplierRepayment",
            idempotency_required=True,
            if_match_required=True,
            response_codes={
                409: (
    ...
    p["/inventory/adjustments"] = {
        "post": post_op(
            ...
            request_schema="StockAdjustmentRequest",
            responses_201="StockMovement",
            idempotency_required=True,
            if_match_required=True,
            response_codes={
                409: (
    ...
    p["/production-runs"] = {
        ...
        "post": post_op(
            "createProductionRun",
            "Create production run in Draft.",
            tags=["Production Runs"],
            capability="production.create",
            request_schema="ProductionRunCreateRequest",
            responses_201="ProductionRun",
            idempotency_required=True,
        ),
    }
    p["/production-runs/{id}"] = {
        ...
        "get": get_op(
            "getProductionRun",
            "Get production run. Use `?include=inputs,cost_lines,output`.",
            "ProductionRun",
            tags=["Production Runs"],
            capability="production.view",
        ),
        "patch": patch_op(
            ...
            "Update Draft production run.",
            tags=["Production Runs"],
            capability="production.edit_own_draft",
            request_schema="ProductionRunPatch",
            if_match_required=True,
        ),
    }
    p["/production-runs/{id}/inputs"] = {
        "parameters": [path_id_("id", "Production run id", "int64")],
        "get": collection_get_op(
            "listProductionInputs",
            "List raw-material inputs.",
            "ProductionInput",
            tags=["Production Inputs"],
            capability="production.view",
            sortable=["line_number"],
        ),
        "post": post_op(
            "addProductionInput",
            "Add a raw-material input to a Draft run. Mutates draft; requires If-Match.",
            tags=["Production Inputs"],
            capability="production.edit_own_draft",
            has_path_id=True,
            request_schema="ProductionInputRequest",
            responses_201="ProductionInput",
            idempotency_required=True,
            if_match_required=True,
            response_codes={
                409: (
    ...
    p["/production-runs/{id}/inputs/{input_id}"] = {
        "parameters": [
            path_id_("id", "Production run id", "int64"),
            path_id_("input_id", "Input id", "int64"),
        ],
        "patch": patch_op(
            "updateProductionInput",
            "Update a Draft input.",
            tags=["Production Inputs"],
            capability="production.edit_own_draft",
            request_schema="ProductionInputRequest",
            if_match_required=True,
    ...
    p["/production-runs/{id}/cost-lines"] = {
        "parameters": [path_id_("id", "Production run id", "int64")],
        "get": collection_get_op(
            "listProductionCostLines",
            "List overhead cost lines.",
            "ProductionCostLine",
            tags=["Production Cost Lines"],
            capability="production.view",
            sortable=["line_number"],
        ),
        "post": post_op(
            "addProductionCostLine",
            "Add an overhead cost line to a Draft run. Mutates draft; requires If-Match.",
            tags=["Production Cost Lines"],
            capability="production.edit_own_draft",
            has_path_id=True,
            request_schema="ProductionCostLineRequest",
            responses_201="ProductionCostLine",
            idempotency_required=True,
            if_match_required=True,
        ),
    }
    p["/production-runs/{id}/cost-lines/{line_id}"] = {
        "parameters": [
            path_id_("id", "Production run id", "int64"),
            path_id_("line_id", "Cost line id", "int64"),
        ],
        "patch": patch_op(
            "updateProductionCostLine",
            "Update a Draft cost line.",
            tags=["Production Cost Lines"],
            capability="production.edit_own_draft",
            request_schema="ProductionCostLineRequest",
            if_match_required=True,
        ...
        "delete": delete_op(
            "deleteProductionCostLine",
            "Delete a Draft cost line.",
            tags=["Production Cost Lines"],
            capability="production.edit_own_draft",
            idempotency_required=True,
        ),
    }
    p["/production-runs/{id}/post"] = {
        "post": post_op(
            "postProductionRun",
            ...
            tags=["Production Runs"],
            capability="production.post",
            has_path_id=True,
            responses_200="ProductionRun",
            idempotency_required=True,
            if_match_required=True,
            response_codes={
                409: (
    ...
    p["/production-runs/{id}/cancel"] = {
        "post": post_op(
            ...
            has_path_id=True,
            request_schema="ProductionCancelRequest",
            responses_200="ProductionRun",
            idempotency_required=True,
            if_match_required=True,
        )
    ...
    p["/manual-entries"] = {
        "get": collection_get_op(
            "listManualEntries",
            "List manual income/expense entries.",
            "ManualEntry",
            tags=["Manual Entries"],
            capability="manual_entry.view",
            filterable_extra={
                "filter[entry_type]": {
        ...
        "post": post_op(
            "createManualEntry",
            "Create manual income or expense entry. Cash is moved atomically.",
            tags=["Manual Entries"],
            capability_dual=(
                "manual_entry.create_income",
                "manual_entry.create_expense",
            ),
            request_schema="ManualEntryRequest",
            responses_201="ManualEntry",
            idempotency_required=True,
            response_codes={
                400: (
                    "Bad Request",
    ...
    p["/manual-entries/{id}/cancel"] = {
        "post": post_op(
            "cancelManualEntry",
            "Cancel a manual entry. Creates a reversing cash_movement.",
            tags=["Manual Entries"],
            capability="manual_entry.cancel",
            has_path_id=True,
            request_schema="ManualEntryCancelRequest",
            responses_200="ManualEntry",
            idempotency_required=True,
            if_match_required=True,
        )
    }
    ...
    # ===== Reports =====
    # F.1 — the four report endpoints whose contracts were finalized
    # in the F.1 audit. Each gets a tailored parameter set (per the
    # audit decision: aggregate endpoints get period params only;
    # paged-list endpoints get the full standard list-query set per
    # API-Architecture §4.8). Later F.2-F.4 reports follow the
    # same shape but use the default tuple form below.
    report_endpoints = [
        # F.1 — sales: aggregate. Period params only (no pagination).
        _ReportEndpoint(
            slug="sales",
            op_id="salesReport",
            desc="Sales report (revenue, COGS, gross, count, avg ticket).",
            schema="SalesReportResponse",
            capability="report.view",
            params=(_PERIOD_PARAM, _FROM_PARAM, _TO_PARAM, _COMPARE_TO_PARAM),
        ),
        # F.1 — purchases: paged list + optional comparison (D-7).
        _ReportEndpoint(
            slug="purchases",
            op_id="purchasesReport",
            desc="Purchase report.",
            schema="PurchasesReportResponse",
            capability="report.view",
            params=(
                _PERIOD_PARAM,
                _FROM_PARAM,
                ...
                {"$ref": "#/components/parameters/Sort"},
                {"$ref": "#/components/parameters/Q"},
                _FILTER_PARAM("filter_supplier_id", "filter[supplier_id]"),
                _FILTER_PARAM("filter_lifecycle_status", "filter[lifecycle_status]"),
                _FILTER_PARAM("filter_posted_by", "filter[posted_by]"),
            ),
        ),
        # F.1 — inventory: aggregate. Period params only (period
        # accepted but ignored — point-in-time snapshot per the
        # audit decision).
        _ReportEndpoint(
            slug="inventory",
            op_id="inventoryReport",
            desc="Inventory valuation + low-stock summary.",
            schema="InventoryReportResponse",
            capability="report.view",
            params=(_PERIOD_PARAM, _FROM_PARAM, _TO_PARAM, _COMPARE_TO_PARAM),
        ),
        # F.1 — inventory-movements: paged list. compare_to is
        # accepted for contract parity but the response is
        # PagedResponse (no comparison field).
        _ReportEndpoint(
            slug="inventory-movements",
            op_id="inventoryMovementsReport",
            desc="Stock movement history.",
            schema="PagedResponse",
            capability="report.view",
            params=(
                _PERIOD_PARAM,
                _FROM_PARAM,
                ...
                {"$ref": "#/components/parameters/Sort"},
                {"$ref": "#/components/parameters/Q"},
                _FILTER_PARAM("filter_product_id", "filter[product_id]"),
                _FILTER_PARAM("filter_trigger", "filter[trigger]"),
                _FILTER_PARAM("filter_reference_type", "filter[reference_type]"),
                _FILTER_PARAM("filter_reference_id", "filter[reference_id]"),
            ),
        ),
    ]
    for entry in report_endpoints:
        p[f"/reports/{entry.slug}"] = {
            "get": get_op(
                entry.op_id,
                entry.desc,
                entry.schema,
                ...
                parameters=list(entry.params),
            )
        }
    # Remaining reports (F.2-F.4) keep the original period-only
    # parameter block; their full contracts are scoped to later
    # milestones and should not be expanded by F.1.
    later_report_endpoints = [
        (
            "sales-returns",
            "salesReturnsReport",
            "Sales return report.",
        ...
        (
            "production",
            "productionReport",
            "Production runs with cost lines.",
            "PagedResponse",
            "report.view",
        ),
        (
        ...
        (
            "refunds",
            "refundsReport",
            "Refunds (customer disbursements).",
            "PagedResponse",
            "report.view",
        ),
        (
            "supplier-repayments",
            "supplierRepaymentsReport",
            "Supplier repayments.",
            "PagedResponse",
            "report.view",
        ),
        ...
        (
            "supplier-receivables",
            "supplierReceivables",
            "Open Supplier Receivable per purchase.",
            "PagedResponse",
            "finance.view_payables_receivables",
        ),
        (
    ...
    # F.1 endpoints already iterated above. F.2+ endpoints keep the
    # original period-only parameter block; their full contracts are
    # scoped to later milestones and should not be expanded by F.1.
    for slug, op_id, desc, schema, cap in later_report_endpoints:
        p[f"/reports/{slug}"] = {
            "get": get_op(
                op_id,
                desc,
    ...
    p["/dashboard"] = {
        "get": get_op(
            ...
            "DashboardResponse",
            tags=["Dashboard"],
            capability="finance.view_profit",
            parameters=[
                {"$ref": "#/components/parameters/Period"},
                {"$ref": "#/components/parameters/From"},
                {"$ref": "#/components/parameters/To"},
    ...
    p["/exports"] = {
        "post": post_op(
            ...
            request_schema="ExportCreateRequest",
            responses_201="ExportJob",
            idempotency_required=True,
        )
    }
    p["/exports/{id}"] = {
    ...
    p["/notifications"] = {
        "get": collection_get_op(
            "listNotifications",
            "List in-app notifications (computed on read for low-stock).",
            "Notification",
            tags=["Notifications"],
    ...
    p["/notifications/{id}/mark-read"] = {
        "post": post_op(
            "markNotificationRead",
            "Mark notification as read.",
            tags=["Notifications"],
            capability="notification.mark_read",
            has_path_id=True,
            responses_204=None,
        )
    }
    p["/notifications/mark-all-read"] = {
        "post": post_op(
    ...
    p["/settings"] = {
        ...
        "patch": patch_op(
            ...
            request_schema="SettingsPatch",
            responses_200="SettingsMap",
            if_match_required=True,
            idempotency_required=True,
        ),
    }
...
def build_openapi():
    ...
    doc["info"] = OrderedDict(
        [
            ...
            (
                "summary",
                "REST + JSON contract for the single-business web-based POS and operations platform.",
            ),
            (
                "description",
                (
    ...
    doc["tags"] = [
        ...
        {
            "name": "Users",
            "description": "User accounts, password reset, unlock, capability grants.",
        },
        {"name": "Roles", "description": "Role management and capability assignment."},
        {
            "name": "Capabilities",
            "description": "Canonical capability catalog (read-only).",
        },
        ...
        {
            "name": "Products",
            "description": "Product master, price history, valuation.",
        },
        {"name": "Contacts", "description": "Customers and suppliers."},
        {
            "name": "Payment Methods",
            "description": "Cash, bank transfer, e-wallet, other.",
        },
        ...
        {
            "name": "Sales",
            "description": "Sales/POS lifecycle: draft, post, pay, cancel, return.",
        },
        {"name": "Sale Lines", "description": "Lines on a sale (sub-resource)."},
        {
            "name": "Sale Payments",
            "description": "Payment allocations on a sale (sub-resource).",
        },
        {
            "name": "Sales Returns",
            "description": "Physical goods returns against sales.",
        },
        {
            "name": "Refunds",
            "description": "Cash disbursements to customers (cash-out, never income).",
        },
        {
            "name": "Purchases",
            "description": "Purchase lifecycle: draft, receive, pay, cancel, return.",
        },
        {
            "name": "Purchase Lines",
            "description": "Lines on a purchase (sub-resource).",
        },
        {
            "name": "Purchase Payments",
            "description": "Payment allocations on a purchase (sub-resource).",
        },
        {
            "name": "Purchase Shipping",
            "description": "Landed cost shipping on a purchase (sub-resource).",
        },
        {
            "name": "Purchase Returns",
            "description": "Physical goods returns to suppliers.",
        },
        {
            "name": "Supplier Repayments",
            "description": "Cash received from suppliers (clears Supplier Receivable).",
        },
        {
            "name": "Inventory",
            "description": "Stock movements, per-product stock summary, low-stock list.",
        },
        {
            "name": "Stock Movements",
            "description": "Immutable stock movement ledger (read-only).",
        },
        {
            "name": "Production Runs",
            "description": "Raw-to-finished production runs and cost lines.",
        },
        {
            "name": "Production Inputs",
            "description": "Raw materials consumed (sub-resource).",
        },
        {
            "name": "Production Cost Lines",
            "description": "Overhead costs (sub-resource).",
        },
        {"name": "Manual Entries", "description": "Manual income and expense entries."},
        {
            "name": "Cash Movements",
            "description": "Immutable cash journal (read-only).",
        },
        {"name": "Reports", "description": "In-app reports."},
        {
            "name": "Dashboard",
            "description": "Dashboard KPIs, trends, best-sellers, expense breakdown.",
        },
