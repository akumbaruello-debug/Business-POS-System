...
"""
import yaml
with open(r'C:/Users/ratus/Desktop/ello/projects/Business-POS-System/openapi.yaml', 'r', encoding='utf-8') as f:
...
lifecycle = ['post','cancel','return','refund','repayment','adjust','login','reset','unlock','grant','revoke']
missing_idem = []
for p, methods in doc.get('paths', {}).items():
    for m, op in methods.items():
        if m != 'post':
        ...
        has = any((pr.get('$ref','').endswith('IdempotencyKeyRequired')) for pr in params if isinstance(pr, dict))
        if not has:
...
write_verbs = ['post','cancel','return','refund','adjust','grant','revoke','mark']
missing_ifmatch = []
for p, methods in doc.get('paths', {}).items():
    for m, op in methods.items():
        if m != 'post':
        ...
        if not any(k in oid for k in write_verbs):
            continue
        params = op.get('parameters', []) or []
        has = any((pr.get('$ref','').endswith('IfMatchRequired')) for pr in params if isinstance(pr, dict))
        if not has:
...
server_derived = {
    'total_amount','paid_amount','outstanding','payment_state','ar','ap','crl','srec',
    'inventory_value','moving_average_unit_cost','on_hand_quantity','low_stock',
    'line_total','line_subtotal','allocated_shipping','finished_unit_cost',
    'total_raw_cost','total_overhead_cost','unit_cost_snapshot','cogs_total_snapshot',
    'balance','unit_cost_at_movement','total_cost','refundable_amount_snapshot',
    'category_name','direction','reference_type','reference_id',
    'reversal_of_movement_id','reversed_by_movement_id','created_at','updated_at',
    'created_by','posted_at','posted_by','cancelled_by','cancellation_date',
    'cancellation_reason','version','is_negative_stock_fallback','stock_movement_id',
    'returned_selling_price','returned_unit_cost','line_value','negative_stock_fallback_supported',
    'lifecycle_status','posted_by','outstanding','payment_state','ar','ap','crl','srec',
}
# Schemas that are RESPONSE-only (not request), even if they end in 'Input'
response_only = {'SalePaymentInput', 'SaleLineInput', 'PurchaseLineInput', 'ProductionInput',
                 'ProductionCostLineRequest', 'SaleLine', 'PurchaseLine', 'ProductionInput',
                 'ProductionOutput', 'ProductionCostLine'}
...
for name, schema in doc.get('components', {}).get('schemas', {}).items():
    if not (name.endswith('Request') or name.endswith('Patch') or name.endswith('Input')):
        continue
